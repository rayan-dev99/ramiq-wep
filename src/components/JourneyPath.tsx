...
const JOURNEY_OBJECTS = [
  // Student Path side
  { type: "backpack", x: 1300, y: 750, size: 100, scrollOffset: 0.1 },
  { type: "student-card", x: 920, y: 1450, size: 100, scrollOffset: 0.2 },
  { type: "headphones", x: 1140, y: 2250, size: 100, scrollOffset: 0.3 },

  // Teacher Path side
  { type: "teacher-badge", x: 160, y: 850, size: 100, scrollOffset: 0.15 },
  { type: "pen", x: 180, y: 1550, size: 100, scrollOffset: 0.25 },
  { type: "calendar", x: 320, y: 2350, size: 100, scrollOffset: 0.35 },

  // Unified Path side
  { type: "location-pin", x: 860, y: 3250, size: 100, scrollOffset: 0.4 },
  { type: "message-bubble", x: 550, y: 4150, size: 100, scrollOffset: 0.5 },
  { type: "book", x: 840, y: 4950, size: 100, scrollOffset: 0.6 },
];
...
export default function JourneyPath() {
  ...
  return (
    ...
    >
      ...
      {/* Premium 3D Journey Path Objects */}
      {JOURNEY_OBJECTS.map((obj, idx) => (
        <div
          key={idx}
          className="absolute select-none pointer-events-none transition-all duration-300 hidden md:block animate-fade-in"
          style={{
            left: `${obj.x * scales.x}px`,
            top: `${obj.y * scales.y}px`,
            width: `${obj.size}px`,
            height: `${obj.size}px`,
            transform: "translate(-50%, -50%)",
          }}
        >
          <ThreeDModel type={obj.type as any} scrollOffset={obj.scrollOffset} />
        </div>
      ))}
    </div>
  );
